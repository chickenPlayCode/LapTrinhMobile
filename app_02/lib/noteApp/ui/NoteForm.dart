...
class _NoteFormScreenState extends State<NoteFormScreen> {
  ...
  void initState() {
    ...
    if (color != null) {
      try {
        _selectedColor = Color(int.parse('FF$color', radix: 16)); // Thêm FF cho alpha channel
      } catch (e) {
          ...
          child: BlockPicker(
            ...
            onColorChanged: (Color newColor) {
              setState(() {
                ...
                // Lưu màu dưới dạng chuỗi hex 6 ký tự (loại bỏ alpha channel)
                String hexColor = newColor.value.toRadixString(16).padLeft(8, '0').substring(2);
                color = hexColor.substring(2); // Chỉ lấy 6 ký tự RRGGBB
              });
            },
          ),
        ),
                  ...
                  onPressed: () async {
                    if (_formKey.currentState!.validate()) {
                      ...
                      try {
                        ...
                        Navigator.pop(context, true); // Trả về true để báo hiệu đã lưu
                      } catch (e) {
                        _scaffoldMessengerKey.currentState?.showSnackBar(
